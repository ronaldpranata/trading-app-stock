...
export default function PerformanceSection({ stocks }: PerformanceSectionProps) {
  ...
  const { xData, seriesData, performanceData } = useMemo(() => {
    ...
    const seriesData = stocks.map((stock, index) => {
    ...
    const performanceData: PerformanceMetric[] = stocks.map((stock, index) => {
      const filteredData = stock.historicalData.slice(-days);
      ...
      <TableContainer component={Paper} variant="outlined" sx={{ borderRadius: 3 }}>
        <Table>
          ...
          <TableBody>
            <CompareRow 
              label="Price" 
              values={stocks.map(s => s.quote?.price ? `$${s.quote.price.toFixed(2)}` : '-')} 
            />
            <CompareRow 
            ...
            <CompareRow 
              label="Market Cap" 
              values={stocks.map(s => s.fundamentalData?.marketCap)}
              format={(v) => v !== undefined && typeof v === 'number' ? formatNum(v) : '-'}
            />
            <CompareRow 
              label="P/E Ratio" 
              values={stocks.map(s => s.fundamentalData?.peRatio)}
              format={(v) => v !== undefined && typeof v === 'number' && v > 0 ? v.toFixed(1) : '-'}
            />
            <CompareRow 
              label="PEG Ratio" 
              values={stocks.map(s => s.fundamentalData?.pegRatio)}
              format={(v) => v !== undefined && typeof v === 'number' && v > 0 ? v.toFixed(2) : '-'}
              colorize
            ...
            <CompareRow 
              label="EPS Growth" 
              values={stocks.map(s => s.fundamentalData?.epsGrowth)}
              format={(v) => v !== undefined && typeof v === 'number' ? `${v >= 0 ? '+' : ''}${v.toFixed(1)}%` : '-'}
              colorize
            />
            <CompareRow 
              label="ROE" 
              values={stocks.map(s => s.fundamentalData?.roe)}
              format={(v) => v !== undefined && typeof v === 'number' ? `${v.toFixed(1)}%` : '-'}
              colorize
            ...
            <CompareRow 
              label="Profit Margin" 
              values={stocks.map(s => s.fundamentalData?.profitMargin)}
              format={(v) => v !== undefined && typeof v === 'number' ? `${v.toFixed(1)}%` : '-'}
              colorize
              thresholds={{ good: 15, bad: 5 }}
            />
            <CompareRow 
              label="RSI" 
              values={stocks.map(s => s.technicalIndicators?.rsi)}
              format={(v) => v !== undefined && typeof v === 'number' ? v.toFixed(1) : '-'}
              colorize
            ...
            <CompareRow 
              ...
              values={stocks.map(s => s.prediction?.technicalScore)}
            ...
            <CompareRow 
              ...
              values={stocks.map(s => s.prediction?.fundamentalScore)}
              format={(v) => v !== undefined && typeof v === 'number' ? v.toFixed(0) : '-'}
              colorize
              thresholds={{ good: 60, bad: 40 }}
            />
